  ...
  const dayResultsPanel = (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
      <div className="overflow-hidden rounded-2xl border border-slate-200">
        <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 text-sm font-bold text-slate-900">Daily results</div>
        {dayResults.length === 0 ? (
          <div className="p-4 text-sm font-semibold text-slate-700">No completed days yet.</div>
        ) : (
                  ...
                  <div className="flex items-start gap-3">
                    <div className="min-w-0">
                      <div className="text-sm font-extrabold text-slate-900">
                        Day {r.dayNumber} · {r.mode.toUpperCase()}
                      </div>
                      <div className="text-xs font-bold text-slate-600 mt-1">{r.note}</div>
                      {r.mode === "guided" ? (
                        <div className="text-xs font-bold text-slate-600 mt-1">
                          Marginal % of correct: {typeof r.guidedMarginalPct === "number" ? `${r.guidedMarginalPct.toFixed(0)}%` : "—"}
                        </div>
                      ) : null}
                    </div>

                    <div className="ml-auto text-right">
                      <div className="text-sm font-extrabold text-slate
