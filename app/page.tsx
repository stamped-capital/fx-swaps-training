 ...
 * - Never suggest more than 3 incorrect swaps in a day.
 * - Score: correct choices / total choices (shown top + progress + completion modal).
 * - Track daily scores + daily costs vs a best-possible cost target.
 *
 * Guided game:
 * - Always show exactly 4 tiles:
 *    - 1 Best, 1 Marginal, 2 Worse
 * - Option tiles are randomized deterministically so Best isn’t always in one spot.
 * - Scoring is correct/total where correct = Best or Marginal.
 * - Grading is NOT shown immediately — click Reveal after an execution to see Best/Marginal/Worse + why.
 * - If a swap clearly went the wrong way, prompt: “Bad Swap Execution - Trade a Reversal?” (Yes/No side-by-side).
 *   - Yes = redo that execution (reverts blotter impact, adds REVERSAL log, does not refund step)
 *   - No = accept and move on
 *
 * Closing completion modal advances the day/value date in both basic + guided.
 */

type Ccy = string;

// Alphabetical order for blotter.
const CCYS: Ccy[] = ["AUD", "CAD", "CHF", "EUR", "GBP", "JPY", "MXN", "NZD", "PHP", "USD"];

const HORIZON = 15; // t+0..t+14
const MAX_DAYS = 80;

const TOL_USD_EQ = 5_000_000;
...
const BASIC_MAX_BAD_SUGGESTIONS = 3;

const GUIDED_MAX_STEPS = 15;

const FWD_POINTS_SPREAD_BP = 1.5;
...
function invertTrade(t: SwapTrade): SwapTrade {
  return {
    buyCcy: t.sellCcy,
    sellCcy: t.buyCcy,
    notionalBuy: Math.round(t.notionalBuy * t.spot),
...
function makeSwapVsUsdToTargetOneCcy(
  ...
  const deltaUsd = targetUsdEq - curUsd;
  const nearOffset = 0;
  let buyCcy: Ccy;
  ...
  if (deltaUsd >= 0) {
    ...
    notionalBuy = Math.round(wantReduceUsd);
  }
  ...
  const t: SwapTrade = { buyCcy: buy, sellCcy: sell, notionalBuy, nearOffset, farOffset, spot: sp, fwd: fw };
...
function buildBasicPlanForDay(startScenario: Scenario, rates: UsdRates): BasicSuggestion[] {
  ...
  // 1) add up to 3 bad suggestions
  let lastBadPair: string | null = null;
  let badCount = 0;
  for (let i = 0; i < BASIC_MAX_BAD_SUGGESTIONS; i++) {
  ...
  for (let guard = 0; guard < 200; guard++) {
    ...
    if (t0BadnessUsdEq(after.balances, rates) >= t0BadnessUsdEq(s.balances, rates)) continue;
  ...
  // 3) pad to minimum steps WITHOUT exceeding 3 bad suggestions
  let avoid = lastBadPair;
  while (plan.length < BASIC_MIN_STEPS && plan.length < BASIC_MAX_STEPS && badCount < BASIC_MAX_BAD_SUGGESTIONS) {
...
type GuidedTile = SwapTrade & {
  ...
  usdCost: number | null;
  why: string;
  counterAmountLabel: string;
};

function makeGuidedTile(rates: UsdRates, t: SwapTrade, category: GuidedTile["category"], why: string): GuidedTile {
  const usdC = swapUsdCost({ notionalBuy: t.notionalBuy, spot: t.spot, fwd: t.fwd, sellCcy: t.sellCcy }, rates);
  const counterNear = Math.round(t.notionalBuy * t.spot);
...
function buildGuidedFourTiles(s: Scenario, rates: UsdRates, seed: number): GuidedTile[] {
  const farOffset = 2;
  const targetMid = 2_500_000;
  ...
  const marginalBase: SwapTrade = { ...best, notionalBuy: Math.max(1, Math.round(best.notionalBuy * 0.5)) };
  const marginal = clampNotionalBuyToUsdEq(marginalBase, rates) ?? marginalBase;
  ...
  const tiles: GuidedTile[] = [
    makeGuidedTile(rates, best, "Best", `Best: solves ${focus} funding into the target range on t+0 (USD-equivalent).`),
    makeGuidedTile(rates, marginal, "Marginal", `Marginal: moves ${focus} toward target, but only ~half as effective as Best.`),
    makeGuidedTile(rates, worse1, "Worse", `Worse: pushes ${focus} away from target (wrong-way direction).`),
    makeGuidedTile(rates, worse2Trade, "Worse", `Worse: increases today’s funding risk versus the objective.`),
  ];

  return deterministicShuffle(tiles, seed);
}

function computeBestPossibleCostGuided(startScenario: Scenario, rates: UsdRates, seedBase: number) {
  let s = shallowCopyScenario(startScenario);
  let cost = 0;

  for (let step = 0; step < GUIDED_MAX_STEPS; step++) {
    if (solvedT0Long0to5mUsdEq(s.balances, rates)) break;
    const tiles = buildGuidedFourTiles(s, rates, seedBase + step * 97);
  ...
  return (
    <div className="flex items-center justify-between bg-white border border-slate-200 rounded-2xl shadow-sm px-5 py-4">
      <div className="flex flex-col gap-1">
        ...
        <div className="text-xs font-bold text-slate-600">t+0 is cash at bank plus executed trades.</div>
      </div>
      <div className="text-xs text-slate-700 font-semibold">
        Goal: every non-USD t+0 must be LONG and within $0 to $5m USD-equivalent.
      </div>
    </div>
  );
}
...
function InfoModal({ open, title, message, onOk }: { open: boolean; title: string; message: string; onOk: () => void }) {
  if (!open) return null;
  return (
...
export default function Page() {
  ...
  function logTrade(t: SwapTrade, correct?: boolean, mode?: TradeLogItem["mode"], tag?: TradeLogItem["tag"]) {
    const usdC =
      ratesRef.current.status === "ok" ? swapUsdCost({ notionalBuy: t.notionalBuy, spot: t.spot, fwd: t.fwd, sellCcy: t.sellCcy }, ratesRef.current) : NaN;

    const item: TradeLogItem = {
      id: id(),
      label: `${tag ? `${tag} · ` : ""}Buy ${t.buyCcy} / Sell ${t.sellCcy} · ${formatM(t.notionalBuy)} ${t.buyCcy} (t+${t.nearOffset} vs t+${t.farOffset})`,
      usdCost: Number.isFinite(usdC) ? usdC : null,
      timestamp: Date.now(),
      correct,
      mode,
  ...
  function hardResetPerDayState() {
    ...
    setGuidedStep(0);
    setGuidedTiles([]);
    setGuidedLastExec(null);
    setGuidedRevealGrade(false);
  ...
  const [basicIncorrect, setBasicIncorrect] = useState<{ active: boolean; lastScenarioBefore?: Scenario; changedCells?: Set<string>; prevValues?: Record<string, number> }>({
    active: false,
  });
  ...
  function basicUndoAndNext() {
    if (!basicIncorrect.active || !basicIncorrect.lastScenarioBefore) return;
  ...
  useEffect(() => {
    ...
    if (solved) basicFinalizeDayAndRoll();
  ...
  const [guidedLastExec, setGuidedLastExec] = useState<{ tile: GuidedTile; correct: boolean } | null>(null);
  const [guidedRevealGrade, setGuidedRevealGrade] = useState(false);

  const [guidedReversalPrompt, setGuidedReversalPrompt] = useState<{ open: boolean; lastScenarioBefore: Scenario; lastTrade: GuidedTile } | null>(null);

  const swapsLeft = GUIDED_MAX_STEPS - guidedStep;

  function refreshGuidedTiles(currentScenario: Scenario, stepSeed: number) {
    if (ratesRef.current.status !== "ok") return;
    const tiles = buildGuidedFourTiles(currentScenario, ratesRef.current, stepSeed);
  ...
  useEffect(() => {
    if (rates.status !== "ok") return;
    const seedBase = dayNumber * 100000 + 17;
    refreshGuidedTiles(scenario, seedBase + guidedStep * 97);
    setGuidedBestCostTarget(computeBestPossibleCostGuided(scenario, rates, seedBase));
  ...
  function executeGuided(t: GuidedTile) {
    ...
    setGuidedLastExec({ tile: t, correct: isCorrect });
    setGuidedRevealGrade(false);

    // prompt reversal if clearly worse
    if (afterBad > beforeBad) {
      setGuidedReversalPrompt({ open: true, lastScenarioBefore: before, lastTrade: t });
    }

    // new tiles for next decision (deterministic)
    const seedBase = dayNumber * 100000 + 17;
    refreshGuidedTiles(after, seedBase + (guidedStep + 1) * 97);
  }
  ...
  function guidedDoReversalYes() {
    ...
    const seedBase = dayNumber * 100000 + 17;
    refreshGuidedTiles(lastScenarioBefore, seedBase + guidedStep * 97);
  }
  ...
  function guidedFinalizeDayAndRoll(finalMessagePrefix: string) {
  ...
  useEffect(() => {
    ...
    if (solved) {
      guidedFinalizeDayAndRoll("Well done, the value date will now roll.");
      return;
    }

    if (guidedStep >= GUIDED_MAX_STEPS) {
      guidedFinalizeDayAndRoll("You ran out of time, the value date will now roll.");
    }
  ...
  function manualPreviewTrade() {
    const t = buildTradeFromManual();
    if (!t) return;
    setManualPreview(applyFxSwap(scenario, t));
  }

  function manualExecuteTrade() {
    const t = buildTradeFromManual();
    if (!t || ratesRef.current.status !== "ok") return;
    const before = scenario;
    const after = applyFxSwap(before, t);
    setScenario(after);
  ...
  const bestPossibleCostToday = tab === "basic" ? basicBestCostTarget : tab === "guided" ? guidedBestCostTarget : 0;

  const costPanel = (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
      <div className="mt-1 overflow-hidden rounded-2xl border border-slate-200">
  <div className="px-4 py-3 bg-slate-50 border-b">
    {/* cost panel content (kept concise) */}
  </div>
    </div>
  </div>
);
