...
type DayPerformance = {
  ...
  correct: number;
  total: number;
  // In guided: marginalCorrect (correct but not best)
...
/**
 * BASIC GAME: we show a list of suggested swaps (some correct, some incorrect),
 * but we cap the number of incorrect suggestions to 3 so it stays solvable fast.
 *
 * We don’t actually need “suggestions” to compute correctness (that's driven by executions),
 * but players like seeing a small suggestion list (think "what to trade").
 */
function buildSwapSuggestions(
  day: DayScenario,
  maxIncorrect: number
): { id: string; text: string; correct: boolean }[] {
  // Construct a pool of "suggestions" derived from executions + a few decoys
  ...
  // Decoys (incorrect). Make more than needed then cap.
  const decoys = [
  ...
  // Small list: mix + cap size a bit
  const mixed = [...correctSuggestions, ...incorrectSuggestions];
  // Keep it from becoming huge if there are many executions
...
function getDayComparisonMessage(prev?: DayPerformance, curr?: DayPerformance) {
  ...
  // Compare score ratio first; if equal, compare cost variance vs target.
  ...
  // Tie-breaker on cost (lower is better)
...
function makeDays(): DayScenario[] {
  // NOTE: You can expand / tune these scenarios. Costs are toy numbers but consistent:
  // bestOptionId is lowest cost; marginal options are near-best.
  const days: DayScenario[] = [
    {
      dayIndex: 1,
...
export default function Page() {
  ...
  // Guided “reveal grading after click” state
  const [guidedReveal, setGuidedReveal] = useState(false);
  const guidedRevealRef = useRef(false);

  // Wrong execution flow (“Bad Swap Execution - Trade a Reversal?”)
  const [wrongChoicePending, setWrongChoicePending] = useState<{
    executionId: string;
    pickedOptionId: string;
  } | null>(null);

  // Derived daily best possible cost
  ...
  const { correctSoFar, marginalCorrectSoFar, bestCorrectSoFar, actualCostSoFar } = useMemo(() => {
    ...
    for (const ex of day.executions) {
      ...
      if (mode === "basic" && r.isBest) best += 1;
      if (mode === "basic" && r.isMarginal) marginal += 1;
    }
  ...
  const isDayComplete = useMemo(() => {
    return day.executions.every(ex => !!picks[ex.id]);
  }, [day.executions, picks]);

  const dailyScoreStr = useMemo(() => {
    return formatScore(correctSoFar, day.executions.length);
  }, [correctSoFar, day.executions.length]);

  const dailyVariance = useMemo(() => actualCostSoFar - bestPossibleCost, [actualCostSoFar, bestPossibleCost]);

  // BASIC GAME: ensure no more than 3 incorrect suggestions
  const suggestions = useMemo(() => buildSwapSuggestions(day, 3), [day]);

  // Guided: randomized tile distribution (per execution, stable per day)
  const getOptionsForExecution = (ex: Execution) => {
    if (mode !== "guided") return ex.options;
    const seed =
      day.dayIndex * 100000 +
      ex.id.split("").reduce((acc, ch) => acc + ch.charCodeAt(0), 0) * 97;
  ...
  function closeModalAndMaybeAdvanceDay() {
    ...
    // Roll value date when completion modal closed (basic + guided)
    if (isDayComplete) {
  ...
  function onChooseOption(ex: Execution, opt: Option) {
    ...
    if (!correct) {
      // Guided: don’t show grading immediately. For wrong picks, show the “Bad Swap Execution...” box with Yes/No.
      setWrongChoicePending({ executionId: ex.id, pickedOptionId: opt.id });
      openInfoModal(
        "Bad Swap Execution - Trade a Reversal?",
        <div className="space-y-3">
          <div className="text-sm text-slate-700">
            You executed a bad swap. Would you like to trade a reversal?
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button
              className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-slate-50"
              onClick={() => {
                // YES: keep your existing logic “as it is” conceptually — we model it as allowing a redo:
                // Remove the pick so the user can re-choose (a reversal / correction attempt).
                setModalOpen(false);
                setWrongChoicePending(null);
                setPicks(prev => {
                  const copy = { ...prev };
            ...
            <button
              ...
              onClick={() => {
                // NO: accept the bad execution and move on.
                setModalOpen(false);
                setWrongChoicePending(null);
              }}
            >
    ...
    // Correct pick: in guided mode don’t reveal “grading” in the modal until clicked.
    if (mode === "guided") {
      setGuidedReveal(false);
      guidedRevealRef.current = false;
      ...
      openInfoModal(
        ...
        <div className="space-y-3">
          <div className="text-sm text-slate-700">
            Execution captured. Click below to reveal whether it was best or marginal.
          </div>
          <button
            className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800"
            onClick={() => {
  ...
  // When day completes, show completion modal (includes score & cost variance vs best possible)
  useEffect(() => {
    ...
    const guidedMarginalPct =
      mode === "guided" && curr.correct > 0 ? (curr.marginalCorrect / curr.correct) : 0;
    ...
    openInfoModal(
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2">
          ...
          {mode === "guided" ? (
            <Pill>Marginal share: {formatPct(guidedMarginalPct)}</Pill>
          ) : null}
        </div>
        ...
        <div className="text-sm text-slate-700">
          Close this box to roll forward to the next value date/day.
        </div>
      </div>
    );
  ...
  // Daily scoreboard above matrix (like the cost tracker idea)
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        ...
        {/* Score history above the rate matrix */}
        <div className="space-y-2">
          {scoreHistoryRow ? (
            <PanelCard title="Final Scores by Day">{scoreHistoryRow}</PanelCard>
          ) : null}
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <PanelCard
              ...
              right={
                <div className="flex items-center gap-2">
                  <Pill>Day {day.dayIndex} of {days.length}</Pill>
                        ...
                        <div className="flex items-center gap-2">
                          {done ? (
                            <>
                              ...
                              {mode === "basic" ? (
                                <Pill>{done.isCorrectBasic ? "Correct ✅" : "Wrong ❌"}</Pill>
                              ) : (
                                <Pill>{done.isCorrectGuided ? "Correct ✅" : "Wrong ❌"}</Pill>
                              )}
                            </>
                          ) : (
                            <Pill>Not done</Pill>
          ...
          <div className="space-y-4">
                  ...
                  {suggestions.map(s => (
                    ...
                    >
                      ...
                      <div className="text-xs text-slate-600">
                        {s.correct ? "Relevant" : "Decoy (max 3 per day)"}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* BASIC PAGE CHANGE: notes under blotter should be at the bottom of the page */}
            <PanelCard title="Progress">
              <div className="space-y-2 text-sm text-slate-700">
                ...
                <div>
                  Score:{" "}
                  <span className="font-semibold text-slate-900">
                    {formatScore(correctSoFar, day.executions.length)}
                  </span>
                </div>
                <div>
                  Cost vs target:{" "}
                ...
                <div>
                  Variance:{" "}
                  <span className="font-semibold text-slate-900">{formatMoney(dailyVariance)}</span>
                </div>

                {mode === "guided" ? (
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
                    Guided scoring: <span className="font-semibold">Best</span> and{" "}
                    <span className="font-semibold">Marginal</span> count as correct. Day completion shows the marginal share.
                  </div>
                ) : (
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
        ...
        {/* Notes moved to bottom of the page (so blotter + suggestion remain visible together) */}
        <div className="mt-4">
        ...
        <div className="mt-6 text-center text-xs text-slate-500">
          {dayPtr >= days.length - 1 && isDayComplete
            ? "You’ve completed the last available day in this demo set."
            : null}
        </div>
      </div>
    </div>
